...
class InvoiceTBS(models.Model):
    ...
    product_id = fields.Many2one('product.product', string='Product TBS', readonly=True, states={'confirmed': [('readonly',False)]})
    ...
    def action_create_bills(self):
        ...
        invoice_ids = []
        for partner in self.invoice_line_ids.mapped('partner_id'):
            invoice_vals = {
                'type': 'in_invoice',
                'partner_id': partner.related_partner_id.id,
                'date_invoice': self.date_invoice,
                'journal_id': self.journal_id.id,
                'account_id': partner.related_partner_id.property_account_payable_id.id,
            ...
            for line in self.invoice_line_ids.filtered(lambda x: x.partner_id.id==partner.id):
                invoice_line_vals = {
                    ...
                    'invoice_line_tax_ids': partner.related_partner_id.default_taxes_ids and [(6,0,partner.related_partner_id.default_taxes_ids.ids)] or [],
                    'name': line.name,
                }
